...
class JobRepositories {
  ...
  async createJob({
    ...
    const query = {
      ...
      INSERT INTO jobs(
        ...
        is_salary_visible, status, company_id, category_id, user_id, 
    ...
    `,
      values: [
        id, title, description, jobType, experienceLevel, locationType, locationCity,
        salaryMin, salaryMax, isSalaryVisible, status, companyId, categoryId, userId,
        createdAt, updatedAt,
      ],
    };
  ...
  async getJobs({ title, company_name } = {}) {
    ...
    if (title) { 
        conditions.push(`LOWER(j.title) LIKE $${i++}`); 
        values.push(`%${title.toLowerCase()}%`); 
    }
    if (company_name) { 
        conditions.push(`LOWER(c.name) LIKE $${i++}`); 
    ...
    const query = {
      text: `
        SELECT 
          j.id, j.title, j.description, j.job_type, j.experience_level, 
    ...
    if (!company_name) {
      return result.rows.map((row) => {
        const { company_name, ...jobWithoutCompanyName } = row;
        return jobWithoutCompanyName;
      });
    }
  ...
  async getJobsByCompanyId(companyId) {
    const query = {
      ...
      text: `SELECT id, title, description, job_type, experience_level, location_type, location_city, salary_min, salary_max, is_salary_visible, status, company_id, category_id 
